...
function readProtocolList(req, res, next) {
    ...
    logger.debug(context, 'List protocols request with args %j', JSON.stringify(req.query));
    ...
    // FIXME: revert PR#231 until will be fix
    ...
    async.series([query.exec.bind(query), Protocol.model.count.bind(Protocol.model, condition)], function (
        ...
        if (error) {
            logger.error(context, 'error: %j', error);
            next(error);
        } else {
            logger.debug(context, 'results of query: %j', results);
...
function deleteProtocol(req, res, next) {
    protocols.remove(req.params.id, function (error) {
        if (error) {
