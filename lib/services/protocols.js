...
function getConfiguration(apikey, resource, protocol, callback) {
    var condition = {
            apikey: apikey,
            resource: resource,
            protocol: protocol
...
function saveConfiguration(protocol, iotagent, resource, configuration, oldConfiguration, callback) {
...
function processConfiguration(protocol, iotagent, resource, configuration, callback) {
    getConfiguration(configuration.apikey, resource, protocol, function(error, oldConfiguration) {
        if (error && error.name === 'DEVICE_GROUP_NOT_FOUND') {
            saveConfiguration(protocol, iotagent, resource, configuration, null, callback);
        } else if (error) {
            callback(error);
        } else {
            saveConfiguration(protocol, iotagent, resource, configuration, oldConfiguration, callback);
        }
    });
}
...
function saveProtocol(req, res, next) {
    ...
    actions.push(apply(cleanConfigurations, protocolObj.protocol, protocolObj.iotagent, protocolObj.resource));

    if (req.body.services) {
        actions.push(apply(async.map,
            req.body.services,
            apply(processConfiguration, protocolObj.protocol, protocolObj.iotagent, protocolObj.resource)));
    }
