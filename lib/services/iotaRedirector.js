...
function extractProtocols(logger, body) {
    const collectionName = guessCollection(body);
    let protocols;

    logger.debug('Extracting protocols from body for collection [%s]', collectionName);
    ...
    if (collectionName) {
        ...
        logger.debug('Protocols found: %j', protocols);
...
function queryParamExtractor(req, res, next) {
    ...
    if (req.query.protocol) {
        logger.debug('Protocol found in the query parameters [%s]', req.query.protocol);
        req.protocolId = [req.query.protocol];
    } else if (req.method !== 'GET') {
        req.splitAndRedirect = true;
        req.protocolId = extractProtocols(logger, req.body);
    }
...
function getProtocols(req, res, next) {
    ...
    if (req.protocolId) {
        logger.debug('Finding objects for the following protocols: %j', req.protocolId);
        ...
        logger.debug('Finding objects for all the protocols');
        ...
        protocols.list(function (error, protocols) {
            if (error) {
                ...
                logger.debug('Finding objects for this protocol: %j', req.protocolId);
                next();
            }
        });
    }
...
function createRequest(req, protocol, body) {
    ...
    logger.debug('creating request using protocol:\n\n%j\n', protocolObj);
...
function createRequests(req, res, next) {
    ...
    if (req.splitAndRedirect) {
        ...
        logger.debug('Creating request for multiple elements');

        req.requests = req.protocolId.map(mapToBody.bind(null, split));
    } else {
        logger.debug('Creating request for redirecting single requests');
        req.requests = [];
...
function processRequests(req, res, next) {
    ...
    function sendRequest(options, callback) {
        logger.debug('Sending redirection with following options: %j', options);
        ...
        request(options, function (error, response, body) {
            if (error) {
                ...
                logger.error('REDIRECTION-001: Error found redirecting requests to [%s]: %j', options.uri, error);
                ...
                logger.debug('Response body from the redirected request parsed: \n\n%j\n', body);
    ...
    function combineResults(results) {
        ...
        for (let i = 0; i < results.length; i++) {
            logger.debug('results[%s][1] %j ', i, results[i][1]);
    ...
    function requestHandler(error, results) {
        let combinedResult;
        if (error) {
            logger.error('The redirection ended up in error: ', error);
            next(error);
        } else {
...
function handleError(error, req, res, next) {
    ...
    //TBD: logger.warn('Error [%s] handling request: %s', error.name, error.message);
