...
function createRequest(req, protocol, body) {
    ...
    req.query.resource = protocolObj.resource;
...
function splitUpdates(req, res, next) {
    function splitUpdate(previous, request) {
        var currentRequest,
            body = JSON.parse(request.body);

        for (var i = 0; i < body.services.length; i++) {
            currentRequest = _.clone(request);
            currentRequest.json = _.clone(body.services[i]);

            if (body.services[i].apikey) {
                currentRequest.qs = _.clone(currentRequest.qs);
                currentRequest.qs.apikey = body.services[i].apikey;
            }

            previous.push(currentRequest);
        }

        return previous;
    }

    if (req.method === 'PUT' && req.body && req.body.services) {
        req.requests = req.requests.reduce(splitUpdate, []);
    }
    ...
    function sendRequest(options, callback) {
        ...
        request(options, function(error, response, body) {
            if (error) {
                ...
                logger.error(context, 'Wrong status code detected [%s] redirecting request.', response.statusCode);
