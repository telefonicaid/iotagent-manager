...
function getProtocols(req, res, next) {
    ...
    if (req.protocolId) {
        logger.debug(context, 'Finding objects for the following protocol: %j', req.protocolId);
        ...
        logger.debug(context, 'Finding objects for all configured protocols');
        configurations.list(req.headers['fiware-service'], req.headers['fiware-servicepath'], function (
            error,
            configurations
        ) {
            if (error) {
                ...
                logger.debug(
                    context,
                    'configurations found for path %s%s: %j',
                    req.headers['fiware-service'],
                    req.headers['fiware-servicepath'],
                    configurations
                );
                req.protocolId = configurations.services.map(extractId);
                req.protocolObj = configurations.services.reduce(addProtocolSync, {});
                logger.debug(context, 'Finding objects for these protocols: %j', req.protocolId);
                next();
            }
        });
    }
...
function processRequests(req, res, next) {
    ...
    function requestHandler(error, results) {
        ...
        if (error) {
            ...
            if (statusCodes.length === 0 && results.length === 0 && req.method === 'GET') {
                // no results (request was redirected to nobody) -> no status
                logger.info(context, 'request was redirected to nobody ');
                res.status(200).json({});
