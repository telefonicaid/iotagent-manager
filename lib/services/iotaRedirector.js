...
function processRequests(req, res, next) {
    ...
    function sendRequest(options, callback) {
        ...
        request(options, function(error, response, body) {
            if (error) {
                logger.error(context, 'REDIRECTION-001: Error found redirecting requests to [%s]: %j',
                    options.url,
                    error);
                ...
                try {
                    ...
                    logger.error(context,
                        'REDIRECTION-003: Error parsing response body from the redirected request: %s', e);
