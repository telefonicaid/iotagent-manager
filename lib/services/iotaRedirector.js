...
function processRequests(req, res, next) {
    ...
    function combineResults(results) {
        ...
        // Remove duplicated devices provided by SMS and Repsol IoTAgents
        // for backward compability with old IOTAM cpp
        if (collectionName === 'devices') {
            var finalResultUnique = arrUnique(finalResult);
            var totalCountUnique = finalResultUnique.length;
            logger.debug(context, 'Number of result devices %s and unique %s',
                         totalCount, totalCountUnique);
            finalResult = finalResultUnique;
            totalCount = totalCountUnique;
