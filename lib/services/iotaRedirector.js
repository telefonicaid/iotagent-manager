    ...
    function sendRequest(options, callback) {
        ...
        request(options, function (error, response, body) {
            if (error) {
                ...
                let parsedBody;
                let parseError;
                logger.debug(context, 'Response status \n\n%j\n', response.statusCode);
                logger.debug(context, 'Response body \n\n%j\n', body);
                try {
    ...
    function requestHandler(error, results) {
        ...
        if (error) {
            ...
            if (statusCodes.length === 0 && results.length === 0 && req.method === 'GET') {
                logger.info(context, 'no results, redirection for GET was done to nobody: ');
                ...
                logger.info(
                    context,
                    'several statusCodes %j and results %j, redirecting for GET %j',
                    statusCodes,
                    results
                );
                combinedResult = combineResults(results);
                logger.info(
                    context,
                    'several statusCodes %j and combinedResults %j, redirecting for GET %j',
                    statusCodes,
                    combinedResult
                );
                if (combinedResult) {
