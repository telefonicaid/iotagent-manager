...
function processRequests(req, res, next) {
    ...
    function sendRequest(options, callback) {
        logger.debug('Sending redirection with following options: %j');

        request(options, function(error, response, body) {
            if (error) {
                logger.error('Error found redirecting requests: %j', error);

                callback(new errors.TargetServerError(error));
            } else if (response.statusCode !== 200 && response.statusCode !== 201 && response.statusCode !== 204) {
                logger.error('Wrong status code detected [%s] redirecting request.', response.statusCode);
                ...
                return callback(parseError, [response, parsedBody]);
            }
        });
    }
