 ...
 */
function init(logger, host, db, port, username, password, options, callback, fullUri = null) {
    let url;
    let retries = 0;
    let lastError;
...
function configureDb(logger, callback) {
    ...
    if (currentConfig.mongodb?.uri) {
        const options = {};
        logger.info(`Using full MongoDB URI from configuration`);
        init(logger, null, null, null, null, null, options, callback, currentConfig.mongodb.uri);
        return;
    }
